...
function Hero({ ready, onContact }: { ready: boolean; onContact: () => void }) {
  ...
  return <section className="hero" ref={ref} aria-labelledby="hero-title">
    ...
    <div className="hero-bottom"><a href="#projets" className="scroll-link"><span className="circle"><ArrowDown size={18} aria-hidden="true"/></span>Explorer mon univers</a><span className="hero-note">Des identités qui ont du sens.<br/>Et qui font la différence.</span></div>
  </section>;
}
function ProjectCard({ project, onSelect }: { project: Project; onSelect: (project: Project) => void }) {
  ...
  return <Reveal className={`project-card ${project.id}`}>
    <motion.button onHoverStart={() => setHovered(true)} onHoverEnd={() => setHovered(false)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} className="project-art" aria-label={`Découvrir ${project.name}`} onClick={() => onSelect(project)}>
      {project.id === 'bertine' ? <><img className="bertine-character" src="/images/bertine-3.webp" alt="Personnage illustré de la pâtissière Bertine" loading="lazy"/><div className="bertine-brand"><img src="/images/bertine-1.webp" alt="Logo Pâtisserie Bertine" loading="lazy"/><span>Un univers à croquer.</span></div></> : project.id === 'unik' ? <><img className="unik-main" src="/images/unik-2.webp" alt="Affiche Unik Locks dans les tons violets et lilas" loading="lazy"/><img className="unik-secondary" src="/images/unik-4.webp" alt="Carte de fidélité et palette Unik Locks" loading="lazy"/></> : <><img className="maeva-photo" src="/images/maeva-3.webp" alt="Univers doux et chaleureux de Maëva Hubert" loading="lazy"/><div className="maeva-brand"><img src="/images/maeva-cover.webp" alt="Monogramme MH de Maëva Hubert" loading="lazy"/><span>Un retour à soi.</span></div></>}
      <motion.span className="project-open" data-expanded={expanded} initial={false} animate={{ width: expanded ? 216 : 56 }} transition={reduced ? { duration: 0 } : { type: 'spring', stiffness: 240, damping: 25 }}><span className="project-open-label">Découvrir le projet</span><span className="project-open-icon"><ArrowUpRight size={22} aria-hidden="true"/></span></motion.span>
    </motion.button>
    <div className="project-caption"><div><h3>{project.name}</h3></div><span>{project.category}</span></div>
  </Reveal>;
}
function Contact({ onContact }: { onContact: () => void }) {
  return <section id="contact" className="contact"><Reveal><div className="section-top"><span className="eyebrow">La prochaine histoire</span><span className="contact-star" aria-hidden="true"><DecorativeStar /></span></div><h2>Et si on créait<br/><em>la suite ?</em></h2><div className="contact-bottom"><p>Une idée à faire grandir, une marque à révéler ?<br/>Tout commence par une conversation.</p><SignatureLink className="signature-light" onClick={onContact}>Parlons de votre projet</SignatureLink></div></Reveal></section>;
}
export default function App() {
  ...
  return <>
    ...
    <header className="site-header"><a href="#" className="wordmark" aria-label="Laxshmee Koomar, accueil"><BrandLogo /></a><nav aria-label="Navigation principale"><a href="#projets">Projets</a><a href="#apropos">À propos</a><button type="button" className="nav-contact" onClick={onContact}>Contact</button></nav></header>
    <main><Hero ready={ready} onContact={onContact}/>
      <section className="intro"><Reveal><span className="eyebrow">L’intention</span><h2>L’art de rendre simple<br/><em>l’inoubliable.</em><span className="intro-star" aria-hidden="true"><DecorativeStar /></span></h2><div className="intro-bottom"><span className="small-rule"/><p>Je transforme une idée en un univers de marque<br className="desktop-break"/> cohérent, identifiable et vivant.</p><a className="inline-link" href="#projets">Une sélection de projets <ArrowDown size={17} aria-hidden="true"/></a></div></Reveal></section>
      <section id="projets" className="projects"><div className="section-top"><span className="eyebrow">Univers choisis</span><span className="eyebrow">2025 — 2026</span></div><Reveal className="projects-heading"><h2>Chaque marque,<br/><em>une histoire.</em></h2><p>De la première intuition<br/>au dernier détail.</p></Reveal>{projects.map((project) => <ProjectCard key={project.id} project={project} onSelect={setSelected}/>)}</section>
      <section id="apropos" className="about"><Reveal className="about-image"><img src="/images/portrait.webp" alt="Portrait de Laxshmee Koomar" width="1333" height="2000" loading="lazy"/><span>Laxshmee Koomar / KMR Design</span></Reveal><Reveal className="about-copy"><span className="eyebrow">Derrière les idées</span><h2>De la sensibilité.<br/>Du sens.<br/><em>Du caractère.</em></h2><p>Créer une identité, c’est raconter une histoire. De la stratégie à l’image, chaque détail a du sens.</p><p>J’accompagne les marques et les entrepreneures dans la construction et le déploiement de leur univers visuel. De la réflexion stratégique à la production des contenus, je veille à créer des identités cohérentes, singulières et pensées pour durer.</p><Expertise /><SignatureLink onClick={onContact}>Faisons connaissance</SignatureLink></Reveal></section>
