...
function Hero({ ready }: { ready: boolean }) {
  ...
  return <section className="hero" ref={ref} aria-labelledby="hero-title">
    ...
    <div className="hero-right"><p>Directrice artistique &<br/>coordinatrice de projets créatifs.</p><a href={contactHref} className="hero-contact">Créons quelque chose ensemble</a></div>
    <div className="hero-bottom"><a href="#projets" className="scroll-link">Explorer mon univers</a><span className="hero-note">Des identités qui ont du sens.<br/>Et qui font la différence.</span><span className="edition">01 — 04</span></div>
  </section>;
}
function ProjectCard({ project, index, onSelect }: { project: Project; index: number; onSelect: (project: Project) => void }) {
  return <Reveal className={`project-card ${project.id}`}>
    <button className="project-art" aria-label={`Découvrir ${project.name}`} onClick={() => onSelect(project)}>
      ...
      <span className="project-open"><span>Découvrir le projet</span></span>
    </button>
