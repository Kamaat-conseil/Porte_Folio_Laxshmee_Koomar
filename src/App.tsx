...
function Hero() {
  ...
  return <section className="hero" ref={ref} aria-labelledby="hero-title">
    ...
    <motion.h1 id="hero-title" initial={{ opacity: 0, y: reduced ? 0 : 65 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.15, ease: [.22, 1, .36, 1] }}><span>Laxshmee</span> <span className="surname">Koomar<span className="name-dot">.</span></span></motion.h1>
    <motion.div className="portrait-frame" style={{ y: reduced ? 0 : y }} initial={{ clipPath: reduced ? 'inset(0%)' : 'inset(100% 0% 0% 0%)' }} animate={{ clipPath: 'inset(0% 0% 0% 0%)' }} transition={{ duration: 1.3, delay: .15, ease: [.22, 1, .36, 1] }}>
...
export default function App() {
  ...
  return <>
    ...
    <header className="site-header"><a href="#" className="wordmark" aria-label="Laxshmee Koomar, accueil">K<span>MR</span><i>®</i></a><nav aria-label="Navigation principale"><a href="#projets">Projets <span>(02)</span></a><a href="#apropos">À propos</a><a href="#contact" className="nav-contact">Contact <ArrowUpRight size={16}/></a></nav></header>
    <main><Hero/>
    ...
    <footer><a href="#" className="footer-logo">KMR<span>Design</span></a><span>© {new Date().getFullYear()} KMR Design</span><span>Identités singulières. Histoires durables.</span><a href="#">Retour en haut ↑</a></footer>
